export const fetchPlaceBearApi = async () => {
  const getRandomNumber = (min: number, max: number) =>
    Math.floor(Math.random() * (max - min + 1)) + min;

  // Generate an array of 900 numbers within the range of 890 to 950
  const randomNumberArray = Array.from({ length: 900 }, () =>
    getRandomNumber(890, 950),
  );

  // Use the generated numbers to create URLs
  const urls = randomNumberArray.map(
    (number) => `https://placebear.com/${number}/600`,
  );
  try {
